...
public class adapter_fragment_album_gridview extends BaseAdapter {
	private fragment_album_gridview_item item = null;
	private Vector<table_album_gridview_photo_path> pictures = null;
	private LayoutInflater inflater = null;
	private Context context = null;
	
	public adapter_fragment_album_gridview(Vector<table_album_gridview_photo_path> pictures, Context context){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			convertView = inflater.inflate(R.layout.fragment_album_gridview_item, parent, false);
		...
		if(photo_path.getResource_id() > 0){
			...
			if(photo_path.getImage_path()!=null && photo_path.getImage_path().length()>0){
				String imgurl = network_interface_paths.get_project_root+ "img/1/s_" + photo_path.getImage_path();
