...
public class thread_album_get_growth_comment_list extends Thread {
	public static ReentrantLock lock = new ReentrantLock();
	private utils_network_interfaces network = new utils_network_interfaces();
	private Handler handler = null;
	private interface_app_get_baby_growth_comment_resp resp = null;
	private interface_app_get_baby_growth_comment_req req = null;
	private Message message = null;
	
	public thread_album_get_growth_comment_list(Handler handler, interface_app_get_baby_growth_comment_req req) {
	...
	public void run() {
		...
		try {
			lock.lock();
			resp = network.ios_interface_app_get_baby_growth_comment(req);
			if(resp == null){
				message = new Message();
				message.what = message_what_values.activity_album_get_data_timeout;
				handler.sendMessage(message);
			}else{
				List<table_album_growth_comment> list = resp.getList();
				list = list == null ? new ArrayList<table_album_growth_comment>() : list;
...
//				}
				// --
				message = new Message();
				message.what = message_what_values.activity_get_baby_growth_comment_list_success;
				message.obj = resp;
				handler.sendMessage(message);
			}
