...
public class thread_album_add_baby_growth extends Thread {
	public static ReentrantLock lock = new ReentrantLock();
	private utils_network_interfaces network = new utils_network_interfaces();
	private Handler handler;
	private interface_app_add_baby_growth_req req = null;
	private interface_app_add_baby_growth_resp resp = null;
	private Message message = null;
	
	public thread_album_add_baby_growth(Handler handler, interface_app_add_baby_growth_req request){
		this.handler = handler;
		this.req = request;
	}
	
	public void run() {
		System.out.println("--------------开启添加宝宝成长记录线程（"+req.getUser_id()+"）---------------");
		try {
			lock.lock();
			resp = network.interface_app_add_baby_growth(req);
			if(resp == null){// 网络异常
				message = new Message();
				...
				message = new Message();
				message.what = message_what_values.activity_album_request_success;
				message.obj = resp;
				handler.sendMessage(message);
			}
		} catch (Exception e) {
