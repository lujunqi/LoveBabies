...
public class thread_album_get_all_my_baby_list extends Thread {
	public static ReentrantLock lock = new ReentrantLock();
	private utils_network_interfaces network = new utils_network_interfaces();
	private Context context = null;
	private Handler handler = null;
	private interface_app_get_all_my_baby_resp resp = null;
	private interface_app_get_all_my_baby_req req = null;
	private table_album_my_baby t_album_my_baby = null;
	private dbs_baby_my_baby dbs_my_baby = null;
	private Message message = null;
	
	public thread_album_get_all_my_baby_list(Context context, Handler handler, interface_app_get_all_my_baby_req req) {
	...
	public void run() {
		System.out.println("--------------开启查询我所有关注的宝宝线程（"+req.getUser_id()+"）---------------");
		try {
			lock.lock();
			resp = network.ios_interface_app_get_all_my_baby(req);
			if(resp == null){
				message = new Message();
