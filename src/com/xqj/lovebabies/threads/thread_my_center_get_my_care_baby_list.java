...
public class thread_my_center_get_my_care_baby_list extends Thread {
	public static ReentrantLock lock = new ReentrantLock();
	private utils_network_interfaces network = new utils_network_interfaces();
	private Context context = null;
	private Handler handler = null;
	private interface_app_get_my_care_baby_resp resp = null;
	private interface_app_get_my_care_baby_req req = null;
	private table_my_center_my_care_baby t_my_care_baby = null;
	private Message message = null;
	
	public thread_my_center_get_my_care_baby_list(Context context, Handler handler, interface_app_get_my_care_baby_req req) {
	...
	public void run() {
		System.out.println("--------------开启查询我关注的宝宝线程（"+req.getUser_id()+"）---------------");
		try {
			lock.lock();
			resp = network.ios_interface_app_get_my_care_baby(req);
			if(resp == null){
				message = new Message();
				...
				list = list == null ? new ArrayList<table_my_center_my_care_baby>() : list;
...
//				}
				// --
				message = new Message();
				message.what = message_what_values.activity_my_center_get_my_care_baby_success;
				message.obj = resp;
				handler.sendMessage(message);
			}
